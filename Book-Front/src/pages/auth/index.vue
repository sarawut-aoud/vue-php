<template>
  <div id="bg-image">
    <v-card class="mx-auto pa-12 pb-8" width="800" elevation="8" max-width="448" rounded="lg" variant="elevated"
      ...
      <v-text-field v-model="password" @keypress="keyEnter" :append-inner-icon="visible ? 'mdi-eye-off' : 'mdi-eye'"
        :type="visible ? 'text' : 'password'" density="compact" placeholder="Enter your password"
        prepend-inner-icon="mdi-lock-outline" variant="outlined"
        @click:append-inner="visible = !visible"></v-text-field>
      <v-btn @click="handleSubmit" :loading="isLoading" :disabled="isLoading" class="mb-4" color="blue" variant="tonal"
        block rounded="lg">
        เข้าสู่ระบบ
      </v-btn>
      ...
      <v-card-text class="d-flex justify-between align-center w-100">
        <div class="w-100 ">
          <v-btn href="/" variant="text" class="text-blue"><v-icon icon="mdi-home"></v-icon> หน้าแรก</v-btn>
        </div>
        <div class="w-100 text-end">
          <v-btn href="register" variant="text" class="text-blue"> สมัครสมาชิก <v-icon
...
let handleSubmit = async () => {
    isLoading.value = true
      await api.post('/api/auth/authen/login',{
            email:email.value,
          ...
          push.error({
                title:"ERROR",
                message:e?.response?.data?.msg[0]?e?.response?.data?.msg[0]:e?.response?.data?.msg
            })
        })
