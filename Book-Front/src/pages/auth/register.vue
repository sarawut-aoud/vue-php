<template>
    <div id="bg-image">
        <v-card class="mx-auto pa-12 pb-8" width="600" elevation="8" rounded="lg" variant="elevated" color="white">
            <v-card-text>
                <div class="text-subtitle-1 text-medium-emphasis">ชื่อที่แสดง *</div>
                <v-text-field v-model="username" density="compact" variant="outlined"
                    hint="ชื่อที่แสดงต้องมีความยาวระหว่าง 6-32 ตัวอักษร สามารถเปลี่ยนแปลงได้ภายหลัง"
                    prepend-inner-icon="mdi-account-box" persistent-hint></v-text-field>
                <div class="text-subtitle-1 text-medium-emphasis mt-5">อีเมล *</div>
                <v-text-field v-model="email" density="compact" prepend-inner-icon="mdi-email-outline"
                ...
                <v-text-field v-model="password" :append-inner-icon="visible ? 'mdi-eye-off' : 'mdi-eye'"
                    ...
                    prepend-inner-icon="mdi-account-key" type="password"
                ...
                <v-text-field v-model="confirmPassword" :append-inner-icon="visible_confirm ? 'mdi-eye-off' : 'mdi-eye'"
                    :type="visible_confirm ? 'text' : 'password'" density="compact" variant="outlined"
                    prepend-inner-icon="mdi-account-key" type="password"
                    @click:append-inner="visible_confirm = !visible_confirm"></v-text-field>
            ...
            <v-card-text class="text-center mt-4">
                <v-btn @click="handleSubmit" :loading="isLoading" :disabled="isLoading" variant="flat" block
                    rounded="lg" color="primary">
                    {{ isLoading ? 'กำลังตรวจสอบ...' : 'ยืนยัน' }}</v-btn>
                <v-btn href="login" variant="text" block rounded="lg" color="primary" class="mt-5">
                    เข้าสู่ระบบ</v-btn>
            </v-card-text>
        </v-card>
    </div>
...
const { passwordError, confirmPasswordError, validatePassword, validateConfirmPassword } = usePasswordValidation();
